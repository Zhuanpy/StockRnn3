...
from code.Evaluation.StockPool import UpdateFakeStock, UpdateRealStock, ScoreStockPool, open_data_file
from code.downloads.DlStockData import DataDailyRenew as ddr
from code.RunMonitor import monitor
from code.Evaluation.PlotStock import PlotsStock
from code.Evaluation.PlotPoolCount import plot_pool_count
from code.RnnModel.CheckModel import RMHistoryCheck
from code.RnnModel.MonitorModel import RMMonitor
from code.AutoTrade.AutoTrading import TradePoolFaker, TradePoolReal
from code.downloads.DlFundsAwkward import DownloadFundsAwkward, AnalysisFundsAwkward
...
class DownloadPage:
    ...
    def __init__(self):
        ...
        self.root.title('下载数据模块')
        self.root.geometry('300x100')

    def download_stock(self):
        pass

    def download_north_fund(self):
        pass

    def download_fundsAwake(self):
        pass
...
class Application(MenuBar):
    ...
    def zone_top(self):
        ...
        Button(fr1, text='基金重仓数据', width=w2, height=h1, command=DownloadData.download_funds_awake).place(x=110, y=y2)
    ...
    def trade_area(self):  # 策略运行
        ...
        Button(fr, text='更新当天交易', width=w1, height=h1, command=UiFun.update_fake_current_trade).place(x=210, y=yFake)
        Button(fr, text='更新持仓记录', width=w1, height=h1, command=UiFun.update_fake_stock_position).place(x=310, y=yFake)
        Button(fr, text='更新交易记录', width=w1, height=h1, command=UiFun.update_fake_stock_history).place(x=410, y=yFake)
        ...
        Button(fr, text='更新持仓记录', width=w1, height=h1, command=UiFun.update_real_stock_position).place(x=310, y=yReal)
        Button(fr, text='更新交易记录', width=w1, height=h1, command=UiFun.update_real_stock_history).place(x=410, y=yReal)
